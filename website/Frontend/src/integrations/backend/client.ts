...
const {
  ...
  majorityCategory,
  score
} = calculateResults(parsedDistribution);

console.log({
  distribution: probabilities,
  nonZero: nonZeroCategories,
  majorityCategory,
...
return {
  distribution: probabilities,      // full normalized distribution
  nonZero: nonZeroCategories,       // only > 0 probability categories
  category: majorityCategory,       // majority class
  safety: responseData.result,
  score                               // highest probability score
};
...
function calculateResults(distribution: Record<string, number>) {
  ...
  // Majority/highest category
  let highest = { key: "", value: 0 };
  for (const [key, value] of Object.entries(probabilities)) {
    if (value > highest.value) highest = { key, value };
  }

  // Filter all categories that have probability > 0
  const nonZeroCategories = Object.fromEntries(
    Object.entries(probabilities).filter(([_, v]) => v > 0)
  );

  return {
    probabilities,        // full normalized list
    nonZeroCategories,    // categories > 0
    majorityCategory: highest.key,
    score: highest.value  // highest probability value
  };
}
